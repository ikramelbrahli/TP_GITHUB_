          ...
          <div class="row">
            ...
            <div class="col-md-6 col-lg-4">
              ...
              <div class="title-box-d">
                ...
                <a href="${pageContext.request.contextPath}/PaiementServlet?action=ViewAllPayments&id=${user.id_user}" class="btn btn-success btn-lg active" role="button" aria-pressed="true">Consulter vos paiements</a>
              </div>
            </div>
 				 ...
 				 <tbody>
      				...
      				<td><a href = "${pageContext.request.contextPath}/LogementServlet?action=ViewLogement&id=${reservation.id_logement}" button type="button" class="btn btn-outline-dark">Voir logement</button>
      				<c:if test = "${reservation.statut == 'En attente'}" > 
      				<td><a href = "${pageContext.request.contextPath}/ReservationServlet?action=deleteReservation&id=${reservation.id_reservation}" button type="button" class="btn btn-outline-danger">Annuler la réservation</button>
      				</td>
      				</c:if>
      				<c:if test = "${reservation.statut == 'Accepté'}" > 
      				<td><a href = "${pageContext.request.contextPath}/PaiementServlet?action=Pay&id=${reservation.id_reservation}" button type="button" class="btn btn-outline-success">Procéder au paiement en ligne</button>
      				</td>
      				
      				</c:if>
      		<c:if test = "${reservation.statut == 'Payé'}" > 
      				<td><a href = "${pageContext.request.contextPath}/RatingServlet?action=Rate&id=${reservation.id_reservation}" button type="button" class="btn btn-outline-success">Donnez un avis</button>
   				</c:if>
   				 </tr>
    		</c:forEach>
              </tbody>
